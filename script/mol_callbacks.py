...
class LossWeightTrack(Callback):
    def __init__(self, loss_weight):
        super(LossWeightTrack, self).__init__()
        self.loss_weight = loss_weight

    def on_epoch_end(self, epoch, logs=None):
        print("Loss weight: z_mean_log_var is", K.get_value(self.loss_weight['z_mean_log_var']), ", x_pred is", K.get_value(self.loss_weight['x_pred']))
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if self.save_best_only:
            ...
            if self.monitor_op(current, self.best):
                ...
                self.best = current
                # self.model.save(os.path.join(self.p["checkpoint_path"], "model_{}.h5".format("best")))  #.format(epoch)
                self.encoder.save(os.path.join(self.p["checkpoint_path"], "encoder_{}.h5".format("best"))) #.format(epoch)
                self.decoder.save(os.path.join(self.p["checkpoint_path"], "decoder_{}.h5".format("best"))) #.format(epoch)
                # self.encoder.save(
                #     os.path.join(self.p["checkpoint_path"], "encoder_{}.h5".format(epoch)))  # .format(epoch)
                # self.decoder.save(
                #     os.path.join(self.p["checkpoint_path"], "decoder_{}.h5".format(epoch)))  # .format(epoch)
                if self.prop_pred_model is not None:
                    self.prop_pred_model.save(os.path.join(self.p["checkpoint_path"], "prop_pred_{}.h5".format("best"))) #.format(epoch)
                    # self.prop_pred_model.save(
                    #     os.path.join(self.p["checkpoint_path"], "prop_pred_{}.h5".format(epoch)))  # .format(epoch)
            else:
                if self.verbose > 0:
